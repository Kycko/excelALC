...
layout = {
    ...
    'run' :{
        ...
        'suggUI':{ # для диалога исправления ошибок
            ...
            'lblInvalid':'Это значение не подходит',    # используется ли? Заменил всё на сообщения из errInput
            'buttons'   :{'cancel':{'text':'Отмена','style':'danger' ,'padx':0},  # именно в таком порядке для упаковки в фрейм
                          'ok'    :{'text':'OK'    ,'style':'success','padx':7}}
            },
        'finished':{'title'  :'Всего осталось ошибок: ',
                    'buttons':{'openErrors':{'text':'Открыть файл с ошибками' ,'style':'secondary','side':'right'},
    ...
    'actions' :{
        ...
        'checkTitles'  :{
            'type' :'main',
            'btn'  :'📊 Проверить столбцы (пока не работает)',
    ...
    'actionsCfg':{  # настройки конкретных проверок
        ...
        'checkTitles' :{
            'reorder' :{'lbl':'  Расставить столбцы по порядку',
        ...
        'checkPhones' :{
            'noBlanks':{'lbl':'  Вписать в пустые ячейки телефон 79999999999',
...
# записи в журнале
log = {'mainLaunch'  : '[$$1] Файл: $$2',   # 1:время запуска, 2:имя проверяемого файла
       ...
       'suggFinished':{'cancelled':'[$$1] Отменено пользователем: "$$2"',       # 1:тип, 2:что
                       'accepted' :'[$$1] Исправлено вручную: "$$2" -> "$$3"'}, # 1:тип, 2:что, 3:на что

       'finalWrite'  :{'skip' :'Ошибок не было найдено, запись в файл не требуется',
                       'main' :'Результат записан на $$1 лист',             # 1 заменяется на след. ключ sheet
                       'sheet':('тот же','новый')},
       'colorErrors' :{'skip' :'Фоновые цвета ячеек сброшены, ошибки НЕ будут выделены красным (их > 100 шт.)',
                       'done' :'Фоновые цвета ячеек сброшены, ошибки выделены красным: $$1 шт.',          # 1:кол-во ошибок
                       '0'    :'Фоновые цвета ячеек сброшены, ошибок не осталось'},
       'fileSaved'   :'Выполнено сохранение файла'}

# ошибки ввода для suggUI
errInput = {'region' :{'notInList'   :'Отсутствует в списке допустимых'},
