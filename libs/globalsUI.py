  ...
  def __init__(self):
    ...
    self.build = {
      ...
      'il:chkMails'    : _getShared('il:taskBtn',
                                   {'inner':{'cmd'  :{'lmb'      :'chkMails'},
                                             'build':{'text'     : S.UI['tasks']['chkMails']['ilBtn'],
                                                      'bootstyle':'primary'}}}),
      'ilBottom'       :{'type' : 'fr',
      ...
      'rbeEntry:ok'    : _getShared('rbeOkCancel',
                                    {'root':{'rules':{'final':('setFocus')},
                                             ...
                                             'build':{'text': self.icons['done'],
                                                      'bootstyle':'success'}}}),
      'rbeEntry:cancel': _getShared('rbeOkCancel',
                                    {'root':{'cmd'  :{'type':'rbeEntry','lmb':'cancel'},
                                             'build':{'text': self.icons['cancel'],
                                                      'bootstyle':'danger'}}}),
      'rbeEntry:errMsg':{'rules':{'final':('color:lightRed')},
                         'type' : 'lbl',
                         'wxKey': 'rbe:errMsg',
                         'pack' :{'fill':'x','padx':2}},
      'rbe:sep'        :{'type' : 'sep',
      ...
      'rbeVarsLbl'     :{'type' : 'lbl',
                         ...
                         'pack' :{'fill' :'x'}},
      'rbeVars:inner'  :{'rules':{'final':('addSuggList')},
                         'type' : 'fr',
                         'wxKey': 'rbeVars:inner',
                         'pack' :{'fill':'x','pady':3}},
      'rbeVars:item'   :{'rules':{'final':('addSuggItem')},
                         'type' : 'fr',
                         'pack' :{'fill':'x','pady':2},
                         'stash':['rbeVars:itemNum','rbeVars:itemBtn']},
      'rbeVars:itemNum':{'type' : 'lbl',
                         'wxKey': 'rbev:num', # нужен только для правила 'addSuggItem'
                         'pack' :{'side':'left','padx':1}},
      'rbeVars:itemBtn':{'type' : 'btn',
      ...
      're:item'        : _getShared('log',{'root' :{'rules':{'final':('paramsConfig',
                                                                      'returnWidget')}}}),
